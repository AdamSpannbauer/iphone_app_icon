...
ap.add_argument("-c", "--useColorFeatures", default=1, type=int,
                help="use color features in search? (0 if no)")
ap.add_argument("-b", "--useBovwFeatures", default=1, type=int,
                help="use bag of words features in search? (0 if no)")
ap.add_argument("-i", "--useImagenetFeatures", default=1, type=int,
                help="use features generated by imagenet trained resnet (0 if no)")
args = vars(ap.parse_args())

# check if user specified to use no features
# raise error if no features specified
if args['useColorFeatures'] == 0 and args['useBovwFeatures'] == 0 and args['useImagenetFeatures'] == 0:
    raise ValueError(
        '''
        --useColorFeatures    (-c) or 
        --useBovwFeatures     (-b) or 
        --useImagenetFeatures (-i) must be non-zero
        ''')
...
# init empty array to bind features to
data = np.array([], dtype=np.int64).reshape(len(image_paths), 0)

if args['useImagenetFeatures'] != 0:
    print(
        '''
[INFO] BOVW AND COLOR FEATURES WILL BE IGNORED
    TO USE BOVW AND COLOR FEATURES SET --useImagenetFeatures
    TO 0 (ie -i 0)
        ''')
    # read csv color features to array
    net_data = np.genfromtxt('{}/imagenet_features.csv'.format(args["featuresPath"]),
                             delimiter=',')
    # drop first column (first col is image file path)
    net_data = np.delete(net_data, 0, axis=1)

    data = np.hstack((data, net_data))
else:
